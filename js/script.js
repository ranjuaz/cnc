...
// ── GALLERY DATA & RENDERING (4 categories, auto-detects uploaded photos) ──
//
...
// The old fixed images/hero-bg.png reference has been removed from styles.css,
// so right now the hero just shows the plain gradient — nothing to delete by hand.
//
// To set a hero background: upload ONE image with any filename into
// images/hero/ (e.g. images/hero/banner.jpg) and it's picked up automatically,
// the same way the gallery folders work.
//
// Testing locally (not on github.io)? It also checks for a fixed file named
// images/hero-bg.jpg / .jpeg / .png / .webp as a fallback.
let heroBgCache = null;

async function fetchGithubHeroImage(){
  ...
  const images = (Array.isArray(list) ? list : [])
    .filter(f => f.type === 'file' && /\.(jpe?g|png|webp)$/i.test(f.name))
    .sort((a, b) => a.name.localeCompare(b.name, undefined, { numeric: true }));
  heroBgCache = images.length > 0 ? images[0].download_url : '';
  return heroBgCache;
}

async function loadHeroBackground(){
  if (!document.querySelector('.hero-bg')) return; // not on a page with the hero section

  // Primary: ask GitHub what's really inside images/hero/ (any filename works)
  try {
    const src = await fetchGithubHeroImage();
    if (src) {
      document.documentElement.style.setProperty('--hero-bg-image', "url('" + src + "')");
      return;
    }
  } catch (e) {
  ...
  // Fallback: probe a few fixed filenames directly (useful when testing locally)
  ...
  // Nothing found anywhere — --hero-bg-image stays "none", plain gradient shows.
}
